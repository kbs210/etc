...
<body>
	<form id="createform" name="createform"
		<c:if test="${pageNumber!=null}">action="${root}/board/updateOk.do?pageNumber=${pageNumber}"</c:if> 
		<c:if test="${pageNumber==null}">action="${root}/board/updateOk.do"</c:if> 
		method="post"
		...
		<div class="menu" style="border-bottom-width: 0px;">
         ...
         <span> 
            ...
            <input type="button" value="목록보기" onclick="location.href='${root}/board/list.do<c:if test="${pageNumber!=null}">?pageNumber=${pageNumber}</c:if> '">
         </span>
      </div>
