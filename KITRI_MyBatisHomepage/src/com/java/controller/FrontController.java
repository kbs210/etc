...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		while (keyIter.hasNext()) {
			...
			try { // ex) className : java.com.member.command.RegisterCommand
				...
				// handlerClass(java.com.member.command.RegisterCommand)경로에 파일이 실제 존재한다면 해당 객체를
				// 반환(newInstnace)해준다
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			// commandmap에서 cmd(/member/register.do)를 이용해 get한다 ->
			// 객체(java.com.member.command.RegisterCommand)를 가져온다
		...
		if(viewPage != null) {
			...
			if(viewPage.equals("/WEB-INF/view/member/register.jsp")) {
				rd = request.getRequestDispatcher(viewPage); // RequestDispathcer을 통해 viewPage실행
			} else {
